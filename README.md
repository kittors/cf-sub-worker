...
- **格式通吃** — 同一地址按客户端下发 Clash YAML / sing-box JSON / base64 节点列表
- **可观测** — 机场剩余流量、套餐到期倒数直接显示在管理端

## 截图

> 下面四张需要你部署后自己截一下：打开 `/admin`，分别在四个界面截图存到 `docs/`。
> 仓库里的 `docs/demo.html` 是一份带示例数据的静态演示页，直接用浏览器打开也能截。
...
| 分流策略：拖拽排序即调整优先级，可切换编辑对象 | 策略编辑：分流目标、严格模式、引用域名集 |
...
```bash
node test/run.cjs     # 187 项断言
bash deploy.sh        # 部署
```

`worker.js` 单文件承载全部逻辑 —— Workers 部署一个 JS 文件最省事，也免去打包步骤。改完先跑测试再部署。

测试覆盖三种输出格式的合法性与引用完整性、规则优先级、目标解析回退、机场元信息多格式解析、多订阅裁剪、以及一组管理端 UI 的静态断言。最后一组是从踩过的坑固化来的。
